...
contract GnomePlayer {
    ...
    mapping(uint256 => uint256) public freezeTimeStamp;
    ...
    function setFreezeTimeStamp(uint256 tokenId, uint256 _freezeTimeStamp) external onlyAuth {
        freezeTimeStamp[tokenId] = _freezeTimeStamp;
    }
    ...
    function currentHP(uint256 tokenId) public view returns (uint256) {
        ...
        uint256 elapsedTime = (block.timestamp - lastUpdateTime) / SECONDS_PER_DAY;
    ...
    function getFreezeTimeStamp(uint256 gnomeID) public view returns (uint256) {
        return freezeTimeStamp[gnomeID];
    }
    ...
    function canGetBooped(uint256 tokenId) public view returns (bool) {
        if (
            ...
            block.timestamp > freezeTimeStamp[tokenId] &&
        ...
        returns (uint256[] memory, address[] memory, string[] memory, uint256[] memory, uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < signedUpIDs.length; i++) {
            ...
            ownersX[i] = gnomeX_usr[i];
            miHPs[i] = hp[signedUpIDs[i]];
