...
contract GnomesTest is ISJ741, NonblockingLzApp {
    ...
    function _mintFactory(address to, bool burnGnomes) internal virtual returns (uint32 tokenId) {
        unchecked {
            minted++; // Increment the total number of minted tokens
        }
        tokenId = minted; // Assign the newly minted token ID
        ...
        libSJ721.emitTransfer(address(0), to, tokenId); // Emit an event for the token transfer
        if (burnGnomes) {
            libSJ20.emitTransfer(address(0), to, (ONE * bridgeFee) / 100000);
            _totalSupply += (ONE * bridgeFee) / 100000;
            _balanceOf[to] += (ONE * bridgeFee) / 100000;
        } else {
    ...
    function trustAddress(string memory _chainTo, address _otherContract) public onlyAuth {
    ...
    function setLimit(uint256 _buylimit, uint256 _selllimit) public onlyAuth {
        buyLimit = _buylimit;
    ...
    function setGnomeGamePlay(IGnomeGamePlay _gnomeGamePlay) external onlyAuth {
